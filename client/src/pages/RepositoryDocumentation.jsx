...
const generatePDF = () => {
  ...
  try {
    ...
    if ((enhancedAnalysis.buildTools && enhancedAnalysis.buildTools.length > 0) || 
      ...
      if (enhancedAnalysis.buildTools && enhancedAnalysis.buildTools.length > 0) {
        toolsData.push(['Build Tools', enhancedAnalysis.buildTools.map(t => safeRender(t)).join(', ')]);
      }
      
      if (enhancedAnalysis.frameworks && enhancedAnalysis.frameworks.length > 0) {
        toolsData.push(['Frameworks', enhancedAnalysis.frameworks.map(f => safeRender(f)).join(', ')]);
      }
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
          ...
          <div className="bg-[#1A1F2E] rounded-xl p-6 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-white">{basicAnalysis.primaryLanguage || 'N/A'}</p>
              </div>
              <div className="col-span-2">
                <p className="text-xs text-[#94A3B8] mb-1">Description</p>
                <p className="text-white">{metadata.description || 'No description provided'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-[#1A1F2E] rounded-xl p-4">
              ...
              <p className="text-2xl font-bold text-white">{stats.totalFiles || countFiles(analysis?.structure) || 0}</p>
            </div>
            <div className="bg-[#1A1F2E] rounded-xl p-4">
              <p className="text-xs text-[#94A3B8] mb-1">Directories</p>
              <p className="text-2xl font-bold text-white">{stats.totalDirs || countDirs(analysis?.structure) || 0}</p>
            </div>
            <div className="bg-[#1A1F2E] rounded-xl p-4">
              <p className="text-xs text-[#94A3B8] mb-1">Dependencies</p>
              <p className="text-2xl font-bold text-white">{basicAnalysis.totalDependencies || 0}</p>
            </div>
            <div className="bg-[#1A1F2E] rounded-xl p-4">
              <p className="text-xs text-[#94A3B8] mb-1">Contributors</p>
              <p className="text-2xl font-bold text-white">{analysis.contributors?.length || 0}</p>
            </div>
          </div>

          {/* Tech Stack */}
          {aiAnalysis.mainTechnologies && aiAnalysis.mainTechnologies.length > 0 && (
            <div className="bg-[#1A1F2E] rounded-xl p-6 mb-6">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          ...
          <div className="bg-[#1A1F2E] rounded-xl p-6 mb-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-white">{safeRender(aiAnalysis.projectType) || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-[#94A3B8] mb-1">Architecture Style</p>
                <p className="text-white">{safeRender(aiAnalysis.architectureStyle) || 'N/A'}</p>
              </div>
              {aiAnalysis.layers && aiAnalysis.layers.length > 0 && (
                <div>
                  <p className="text-xs text-[#94A3B8] mb-2">Architecture Layers</p>
              ...
              <div className="space-y-3">
                {analysis.contributors.slice(0, 5).map((contributor, index) => (
                  <div key={index} className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-[#94A3B8]">{contributor.contributions} commits</span>
                  </div>
                ))}
              </div>
            </div>
